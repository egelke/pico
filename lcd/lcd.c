...
int lcd_write_str(lcd_device_t *d, const char * str) {
    int strlen = strnlen(str, 1 + MAX_STR_LEN);
    if (strlen > MAX_STR_LEN) return LCD_ERROR_STRSIZE;
    ...
    if (d->emul) {
        ...
        while(*str != '\0') {
            dbuf_len += 6;
            memcpy(p, font_6x8[*str], 6);
            p += 6;
            str++;
        }
    } else {
        dbuf_len = strlen;
        memcpy(intf->dbuf, str, strlen);
    }
    return intf->send_dbuf(intf, &(dp->intf_meta), dbuf_len);
}
